...
# An input this agent is genuinely FOR. It has to be a question the shipped corpus can
# match: retrieval short-circuits before the model when nothing is found, and the test
# would then report a defect that is not there.
LIVE_INPUT = "How do I retrieve multiple records from the documented API?"

# What the stand-in model answers. ResponseGenerateNode parses the reply as JSON and
# ResponseValidateNode checks the citations against the retrieved chunks, so a bare
# sentence is correctly discarded.
MODEL_REPLY = (
    '{"thought": "The records list endpoint returns a page of records.", '
    '"answer": "Use GET /v1/records with the app parameter. [endpoint: /v1/records GET]", '
    '"citations": ["/v1/records GET"], "confidence": 0.9}'
)

# A phrase the degraded answer always contains. Taken from _NO_DOCS_ANSWER in
# src/nodes/response_generate.py so the test breaks when that wording changes.
DEGRADED_SIGNAL = "No relevant documentation found"
